...
def flatten_documents(corpus: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for doc in corpus:
        ...
        # 第 4~7 頁的修改紀錄表
        # 只要 metadata 裡有 revision_records，就算 content_type 沒寫 revision_table，
        # 也一樣拆成每一筆版本紀錄去 embedding / 寫入 Qdrant。
        revision_records = metadata.get("revision_records", []) or []

        if content_type == "revision_table" or revision_records:
...
def build_embedding_text(row: Dict[str, Any]) -> str:
    ...
    major_title = "" if row.get("major_title") is None else str(row.get("major_title", "")).strip()
    minor_title = "" if row.get("minor_title") is None else str(row.get("minor_title", "")).strip()
...
def write_to_qdrant_cloud(
    ...
    qdrant_collection: str = "maintenance_manual_v3",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--collection", default="maintenance_manual_v3")
